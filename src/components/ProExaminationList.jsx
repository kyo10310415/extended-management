...
function ProExaminationList() {
  ...
  useEffect(() => {
    fetchProExaminationStudents()
  }, [monthOffset])

  const fetchProExaminationStudents = async () => {
    try {
      setLoading(true)
      setRefreshing(false) // 初回読み込みの場合はrefreshingをfalseに
      const response = await fetch(`/api/notion/pro-examination?monthOffset=${monthOffset}`)
      const data = await response.json()

      if (data.success) {
        // 調整後月数が17ヶ月目の生徒を取得
        const month17Students = data.data.filter(s => s.adjustedMonths === 17);
        
        // サイクル3のデータ取得（17ヶ月目）
        const cycle3Ids = month17Students.map(s => s.studentId);
      ...
      console.error('Error fetching pro examination students:', err)
  ...
  const handleRefresh = async () => {
    try {
      ...
      // データ再取得
      await fetchProExaminationStudents()
  ...
  const handleUpdate = async (studentId, updatedData) => {
    try {
      ...
      console.log('📝 handleUpdate 呼び出し (Pro延長審査)');
...
export default ProExaminationList
